...
class Eolica(models.Model):
    central = models.OneToOneField('Central', on_delete=models.CASCADE, related_name='eolica')
    ...
    # Costos
    costo_inversion = models.DecimalField(max_digits=20, decimal_places=4, help_text="%")
    ...
    combustible = models.DecimalField(max_digits=15, decimal_places=4)
    transporte = models.DecimalField(max_digits=15, decimal_places=4)
    lubricantes = models.DecimalField(max_digits=15, decimal_places=4)
    agua = models.DecimalField(max_digits=15, decimal_places=4)
    mantenimiento = models.DecimalField(max_digits=15, decimal_places=4)
    control = models.DecimalField(max_digits=15, decimal_places=4)
    servicios = models.DecimalField(max_digits=15, decimal_places=4)
    seguros = models.DecimalField(max_digits=15, decimal_places=4)
    personal = models.DecimalField(max_digits=15, decimal_places=4)
    ...
    def save(self, *args, **kwargs):
        if self.potencia_nominal and self.factor_planta:
            self.energia_bruta_calculada = self.potencia_nominal * self.factor_planta * 8760 / Decimal('1000')
            self.energia_anual_producida = self.potencia_nominal * (self.factor_planta / Decimal('100')) * Decimal('8760')
        ...
        self.costo_fijo_anual = self.inversion_total * (self.costo_inversion/Decimal('100'))
        self.costo_fijo_mw = self.costo_fijo_anual / self.potencia_nominal
        self.costo_fijo_kw = self.costo_fijo_mw / Decimal('1000')

        super().save(*args, **kwargs)

    def __str__(self):
        return f'Eolica: {self.central.nombre}'


# ----------------------------- Class Hidraulica --------------------------------
class Hidraulica(models.Model):
    central = models.OneToOneField('Central', on_delete=models.CASCADE, related_name='hidraulica')
    ...
    costo_variable = models.DecimalField(max_digits=15, decimal_places=6, help_text="USD")
    costo_produccion = models.DecimalField(max_digits=15, decimal_places=6, help_text="USD")
    costo_operacion = models.DecimalField(max_digits=15, decimal_places=6, help_text="USD")
    gastos_administrativos = models.DecimalField(max_digits=15, decimal_places=6, help_text="USD")
    potencia_total = models.DecimalField(max_digits=15, decimal_places=6, help_text="MW")
    costo_administracion = models.DecimalField(max_digits=15, decimal_places=6, blank=True, null=True)
    costo_mantenimiento = models.DecimalField(max_digits=15, decimal_places=6, help_text="USD")
    ...
    def save(self, *args, **kwargs):
        ...
        self.periodos_pago = self.total_periodos - self.periodos_gracia
        self.anios_pago = self.periodos_pago / self.periodos_por_anio if self.periodos_por_anio else None
        self.costo_administracion = self.gastos_administrativos * (self.potencia_nominal / self.potencia_total) / self.energia_anual_producida

        super().save(*args, **kwargs)

    def __str__(self):
        return f'Hidraulica: {self.central.nombre}'
...
class ParametroEolica(ParametroCalculos):
    ...
    def calcular_lcoe(self):
        ...
        try:
            ...
            IN = to_decimal(self.inversion_total)
            CP = to_decimal(self.capital_propio)
            COM = to_decimal(self.combustible)
            TRA = to_decimal(self.transporte)
            LUB = to_decimal(self.lubricantes)
            AGUA = to_decimal(self.agua)
            MANT = to_decimal(self.mantenimiento)
            CONT = to_decimal(self.control)
            SERV = to_decimal(self.servicios)
            SEGU = to_decimal(self.seguros)
            PER = to_decimal(self.personal)
            COI = to_decimal(self.costo_inversion)
            CFA = to_decimal(self.costo_fijo_anual)
            CFMW = to_decimal(self.costo_fijo_mw)
            CFKW = to_decimal(self.costo_fijo_kw)   
            ...
            CF_total_3 = CFMW 
            CF_total_4 = CFKW
            CF_total = CF_total_1 + CF_total_2 + CF_total_3 + CF_total_4
            # Costo variable total
            CV1 = COM * EAP
            CV2 = TRA * EAP
            CV3 = LUB * EAP
            ...
            CV8 = SEGU * EAP
            CV9 = PER * EAP
            CV_total = CV1 + CV2 + CV3 + CV4 + CV5 + CV6 + CV7 + CV8 + CV9
            # Suma de costos 
            costos = CF_total + CV_total
...
class ParametroHidraulica(ParametroCalculos):
    # Costos
    costo_variable = models.DecimalField(max_digits=15, decimal_places=6, blank=True, null=True)
    costo_produccion = models.DecimalField(max_digits=15, decimal_places=6, blank=True, null=True)
    costo_operacion = models.DecimalField(max_digits=10, decimal_places=6, blank=True, null=True)
    gastos_administrativos = models.DecimalField(max_digits=10, decimal_places=6, blank=True, null=True)
    potencia_total = models.DecimalField(max_digits=15, decimal_places=6, blank=True, null=True)
    costo_administracion = models.DecimalField(max_digits=15, decimal_places=6, blank=True, null=True)
    costo_mantenimiento = models.DecimalField(max_digits=15, decimal_places=6, blank=True, null=True)
    ...
    def calcular_lcoe(self):
        ...
        try:
            ...
            cost_P = to_decimal(self.costo_produccion)
            cost_O = to_decimal(self.costo_operacion)
            cost_A = to_decimal(self.costo_administracion)
            cost_M = to_decimal(self.costo_mantenimiento)
            degradacion = to_decimal(self.degradacion) / Decimal('100')
            ...
            # Costos totales
            CF_total_1 = cost_P * EAP
            CF_total_2 = cost_O * EAP
            CF_total_3 = cost_A * EAP
            CF_total_4 = cost_M * EAP
            CF_total = CF_total_1 + CF_total_2 + CF_total_3 + CF_total_4
            # Costo variable total
            CV_total = CV * EAP
            # Suma de costos 
    ...
    def save(self, *args, **kwargs):
        try:
            ...
            if hasattr(self.caso, 'parametrofotovoltaica'):
                ...
                tipo = 'termica'
            elif hasattr(self.caso, 'parametroeolica'):
                parametros = self.caso.parametroeolica
                tipo = 'eolica'
            elif hasattr(self.caso, 'parametrohidraulica'):
                parametros = self.caso.parametrohidraulica
                tipo = 'eolica'
